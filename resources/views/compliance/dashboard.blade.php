...
<head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/antd/5.13.0/antd.min.css">
    <style>
        .navbar { background: linear-gradient(135deg, #1890ff 0%, #722ed1 100%); }
        .status-badge { font-size: 0.85rem; }
        .subscription-badge { font-size: 0.9rem; padding: 0.5rem 1rem; }
        .card { border-radius: 8px; box-shadow: 0 1px 2px 0 rgba(0,0,0,0.03), 0 1px 6px -1px rgba(0,0,0,0.02), 0 2px 4px 0 rgba(0,0,0,0.02); }
...
<body>
        ...
        <div class="container">
            ...
            <div class="d-flex align-items-center gap-3">
                @if(isset($subscription))
                    <span class="badge subscription-badge {{ $subscription === 'FULL' ? 'bg-success' : 'bg-secondary' }}">
                        {{ $subscription }}
                    </span>
                @endif
                ...
                <form method="POST" action="{{ route('logout') }}" class="d-inline">
                    @csrf
                    <button type="submit" class="btn btn-sm btn-outline-light ant-btn">Logout</button>
                </form>
            </div>
        </div>
    </nav>
    ...
    <div class="container">
        @if(isset($subscription))
        <div class="card shadow-sm mb-4 border-primary ant-card">
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <p class="mb-2">
                            ...
                            <span class="ant-tag {{ $subscription === 'FULL' ? 'ant-tag-success' : 'ant-tag-default' }}" style="font-size: 14px; padding: 4px 12px;">
                                {{ $subscription }}
                            </span>
                        </p>
                    </div>
        ...
        <div class="card shadow-sm mb-4 border-{{ $healthScore['status'] === 'Excellent' ? 'success' : ($healthScore['status'] === 'Good' ? 'warning' : 'danger') }} ant-card">
                    ...
                    <div class="col-md-8">
                        ...
                        <ul class="list-unstyled">
                            @foreach($healthScore['breakdown'] as $metric => $score)
                                <li class="mb-1">
                                    <span class="ant-tag {{ $score >= 18 ? 'ant-tag-success' : ($score >= 10 ? 'ant-tag-warning' : 'ant-tag-error') }} me-2">{{ $score }}%</span>
                                    {{ $metric }}
                                </li>
                            @endforeach
                        </ul>
        ...
        <div class="card shadow-sm mb-4 border-info ant-card">
        ...
        @if(isset($subscription) && $subscription === 'MINIMAL')
            <div class="alert alert-warning ant-alert ant-alert-warning">
                <strong>⚠️ MINIMAL Subscription:</strong> You can upload forms manually. Upgrade to FULL for automation features.
            </div>
        @endif

        @if(session('success'))
            <div class="alert alert-success alert-dismissible fade show ant-alert ant-alert-success">
                {{ session('success') }}
        ...
        @if(session('error'))
            <div class="alert alert-danger alert-dismissible fade show ant-alert ant-alert-error">
                {{ session('error') }}
                <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
            </div>
        @endif
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="card shadow-sm mb-4 ant-card">
                    <div class="card-header bg-primary text-white">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('compliance.batch.create') }}" id="batchForm">
                            ...
                            <button type="submit" class="btn btn-primary w-100 ant-btn ant-btn-primary" id="createBatchBtn">
            ...
            <div class="col-md-6">
                @if(session('batch_id'))
                    <div class="card shadow-sm mb-4 border-success ant-card">
                        ...
                        <div class="card-body">
                            ...
                            @if(session('results'))
                                ...
                                <div class="mt-3 d-flex gap-2">
                                    <a href="{{ route('compliance.batch.download', session('batch_id')) }}" class="btn btn-info ant-btn">📥 Download Report</a>
                                    @if(isset($subscription) && $subscription === 'FULL')
                                        <a href="{{ route('compliance.batch.inspectionPack', session('batch_id')) }}" class="btn btn-primary ant-btn ant-btn-primary">📦 Inspection Pack</a>
                                    @endif
                                </div>
                            @elseif(isset($subscription) && $subscription === 'MINIMAL')
                                <p><strong>Status:</strong> <span class="ant-tag ant-tag-warning" id="batchStatus">Awaiting Manual Upload</span></p>
                                <div id="uploadFormsSection" class="mt-3">
                                    <p class="text-muted">Step 1: Upload forms manually</p>
                                    <div id="uploadFormsList"></div>
                                </div>
                                <div class="mt-3 d-flex flex-column gap-2">
                                    <button type="button" class="btn btn-warning ant-btn" id="processUploadsBtn" onclick="processUploads({{ session('batch_id') }})" disabled>
                                        ⚙️ Step 2: Process Uploaded Files
                                    </button>
                                    <a href="{{ route('compliance.batch.download', session('batch_id')) }}" class="btn btn-info ant-btn" id="generateReportBtn" style="display:none;">
                            ...
                            @else
                                ...
                                @if(isset($subscription) && $subscription === 'FULL')
                                    ...
                                    <form method="POST" action="{{ route('compliance.batch.process', session('batch_id')) }}" class="mt-3">
                                        @csrf
                                        <button type="submit" class="btn btn-success w-100 ant-btn ant-btn-primary" style="background: #52c41a; border-color: #52c41a;">⚙️ Process Batch</button>
                                    </form>
                                @endif
                            @endif
                        </div>
                ...
                <div class="card shadow-sm ant-card">
                    <div class="card-header bg-info text-white">
                    ...
                    <div class="card-body">
                        <div class="row text-center">
                            ...
                            <div class="col-4">
                                <h3 class="text-info">{{ collect($batches)->filter(fn($b) => in_array($b->display_status ?? $b->status, ['Completed', 'completed']))->count() }}</h3>
        ...
        <div class="card shadow-sm mt-4 ant-card">
            ...
            <div class="card-body">
                @if(count($batches) > 0)
                    <div class="table-responsive">
                        <table class="table table-hover ant-table ant-table-bordered">
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Section</th>
                            ...
                            <tbody>
                                @foreach($batches as $batch)
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                                $displayStatus = $batch->display_status ?? $batch->status;
                                            @endphp
                                            @if($displayStatus === 'Completed')
                                                <span class="ant-tag ant-tag-success status-badge">Completed</span>
                                            @else
                                                <span class="ant-tag ant-tag-success status-badge">Completed</span>
                                            @endif
                                        </td>
                                        ...
                                        <td>
                                            @php
                                                $displayStatus = $batch->display_status ?? $batch->status;
                                            @endphp
                                            @if(in_array($displayStatus, ['Completed', 'Partially Completed', 'completed']))
                                                <a href="{{ route('compliance.batch.download', $batch->id) }}" class="btn btn-sm btn-info ant-btn">📥 Download</a>
                                            @elseif(in_array($displayStatus, ['Pending', 'pending', 'Failed']))
                                                <a href="{{ route('compliance.batch.download', $batch->id) }}" class="btn btn-sm btn-info ant-btn">📥 Download</a>
                                                @if(isset($subscription) && $subscription === 'FULL')
    ...
    <script>
                ...
                forms.forEach(form => {
                    ...
                    div.innerHTML = `
                        ...
                        <label class="form-check-label" for="form${form.id}">
                            ...
                            <span class="ant-tag ant-tag-${form.priority === 'High' ? 'error' : form.priority === 'Medium' ? 'warning' : 'default'} ms-2">${form.priority}</span>
                        </label>
                    `;
        ...
        @if(session('batch_id') && isset($subscription) && $subscription === 'MINIMAL')
        ...
        @if(session('batch_id') && isset($subscription) && $subscription === 'FULL' && !session('results'))
                            ...
                            if (form) {
                                ...
                                div.innerHTML = `
                                    <a href="/compliance/batch/${previewBatchId}/preview/${form.form_code}"
                                       class="btn btn-sm btn-outline-primary w-100 ant-btn" target="_blank">
        ...
        function checkAllUploaded() {
            ...
            const allUploaded = Array.from(fileInputs).every(input => input.disabled);
            if (allUploaded && fileInputs.length > 0) {
                document.getElementById('processUploadsBtn').disabled = false;
            }
        }

        function processUploads(batchId) {
            const btn = document.getElementById('processUploadsBtn');
            btn.disabled = true;
            btn.innerHTML = '<span class="spinner-border spinner-border-sm"></span> Processing...';

            const csrfToken = document.querySelector('meta[name="csrf-token"]').content;
            fetch(`/compliance/batch/${batchId}/process-uploads`, {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    document.getElementById('batchStatus').textContent = 'Processed';
                    document.getElementById('batchStatus').classList.remove('bg-warning');
                    document.getElementById('batchStatus').classList.add('bg-success');
                    document.getElementById('generateReportBtn').style.display = 'block';
                    alert(data.message);
                } else {
                    alert('Processing failed: ' + data.message);
                    btn.disabled = false;
                    btn.innerHTML = '⚙️ Step 2: Process Uploaded Files';
                }
            })
            .catch(e => {
