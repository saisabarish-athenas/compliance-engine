# STATUTORY REFERENCE STRUCTURE MAP

**Purpose:** Document exact structural components from official government reference PDFs

**Location of Reference PDFs:** `Forms`

**Status:** Template ready for population from actual reference PDFs

---

## EXTRACTION METHODOLOGY

For each form, extract:
1. Exact form title (including capitalization)
2. Act name and section reference
3. Rule reference
4. Header formatting structure
5. Table column order (left to right)
6. Column heading exact wording
7. Footer declaration text
8. Signature block layout
9. Seal positioning
10. Page numbering style
11. Section spacing (in mm or px)
12. Totals row placement and format
13. Static legal text

---

## FACTORIES ACT FORMS (13 Forms)

### FORM_B - Register of Wages

**Reference PDF:** `Forms/Register of wages.pdf`

**Extracted Structure:**
```
FORM TITLE: [Extract exact title from PDF]
ACT REFERENCE: [Extract exact act reference]
RULE REFERENCE: [Extract exact rule reference]

HEADER SECTION:
- Line 1: [Exact text]
- Line 2: [Exact text]
- Alignment: [Left/Center/Right]

TABLE STRUCTURE:
Column 1: [Exact heading] | Width: [%] | Align: [L/C/R]
Column 2: [Exact heading] | Width: [%] | Align: [L/C/R]
Column 3: [Exact heading] | Width: [%] | Align: [L/C/R]
...

TOTALS ROW:
- Label: [Exact text]
- Position: [After all rows / Separate section]
- Format: [Bold / Underline / Background]

FOOTER DECLARATION:
[Exact text from PDF]

SIGNATURE BLOCK:
- Left side: [Date / Prepared by / etc.]
- Right side: [Signature / Name / Designation]
- Seal position: [Below signature / Right of signature]

PAGE NUMBERING:
- Style: [Page X of Y / Page X / None]
- Position: [Top-right / Bottom-center / etc.]

SPACING:
- Top margin: [mm]
- Bottom margin: [mm]
- Left margin: [mm]
- Right margin: [mm]
- Line spacing: [pt]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_10 - Overtime Register

**Reference PDF:** `Forms/Overtime Muster.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_25 - Muster Roll

**Reference PDF:** `Forms/Form 25 – Muster Roll.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_12 - Register of Adult Workers

**Reference PDF:** `Forms/Form 12 - Adult worker register.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_2 - Register of Leave

**Reference PDF:** `Forms/Form 15 - Leave with wages.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_7 - Notice of Periods for Adult Workers

**Reference PDF:** `Forms/Form 2 – Notice of Periods of Work.doc`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_8 - Register of Accidents

**Reference PDF:** `Forms/Form 11 - Accident register.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_11 - Notice of Dangerous Occurrences

**Reference PDF:** `Forms/Form 18A - Report of dangerous occurrence.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_17 - Register of Young Persons

**Reference PDF:** `FORM_17_FACTORIES_ACT.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_18 - Register of Child Workers

**Reference PDF:** `FORM_18_FACTORIES_ACT.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_26 - Notice of Accident

**Reference PDF:** `Forms/Form 18 - Report of accident.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_26A - Notice of Dangerous Occurrence

**Reference PDF:** `Forms/Form 26A –Register of dangerous occurrences.doc`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### HAZARD_REG - Hazardous Process Register

**Reference PDF:** `HAZARD_REG_FACTORIES_ACT.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

## CLRA FORMS (13 Forms)

### FORM_XII - Register of Contractors

**Reference PDF:** `Forms/Register of Contractors (Solar Site Projects).docx`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### CLRA_LICENSE - License Register

**Reference PDF:** `Forms/Contractor License Register (CLRA).docx`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_XIII - Register of Contract Labour

**Reference PDF:** `Forms/Form XIII – Register of Workmen Employed by Contractor.docx`

**Extracted Structure:**
```
FORM TITLE: [Extract from PDF]
ACT REFERENCE: Contract Labour (Regulation and Abolition) Act, 1970
RULE REFERENCE: [Extract exact rule reference]

[Structure to be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_XVI - Register of Wages

**Reference PDF:** `Forms/Register of wages.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_XVII - Register of Deductions

**Reference PDF:** `Forms/Form XX – Register of Deductions (CLRA).docx`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_XIX - Muster Roll

**Reference PDF:** `Forms/Form XVI – Muster Roll (CLRA Version).docx`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_XIV - Register of Workmen

**Reference PDF:** `Forms/Register of Workmen.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_XX - Register of Advances

**Reference PDF:** `Forms/Form XXII – Register of Advances (CLRA).docx`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_XXI - Register of Fines

**Reference PDF:** `Forms/Register of fines.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_XXII - Register of Damage or Loss

**Reference PDF:** `FORM_XXII_CLRA.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_XXIII - Register of Overtime

**Reference PDF:** `Forms/Form XXIII – Register of Overtime (CLRA).docx`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_XXIV - Annual Return

**Reference PDF:** `Forms/Form XXV – Annual Return (Principal Employer).docx`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### FORM_XXV - Half-Yearly Return

**Reference PDF:** `Forms/Form XXIV – Half-Yearly Return (Contractor).docx`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

## SHOPS ACT FORMS (7 Forms)

### SHOPS_FORM_12 - Register of Wages

**Reference PDF:** `Forms/Register of wages.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### SHOPS_FORM_13 - Attendance Register

**Reference PDF:** `SHOPS_FORM_13.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### SHOPS_FORM_1 - Register of Employment

**Reference PDF:** `Forms/Form XIV – Employment Card (CLRA).docx`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### SHOPS_FINES - Register of Fines

**Reference PDF:** `Forms/Register of fines.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### SHOPS_FORM_C - Bonus Register

**Reference PDF:** `Forms/Bonus & Financial Registers.xls`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### SHOPS_UNPAID - Unpaid Wages Register

**Reference PDF:** `Forms/Form C - Unpaid accumulation.pdf`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### SHOPS_FORM_VI - Leave Register

**Reference PDF:** `Forms/Form VI – Holidays Register.xlsx`

**Extracted Structure:**
```
[To be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

## SOCIAL SECURITY FORMS (2 Forms)

### ESI_FORM_12 - Accident Register

**Reference PDF:** `Forms/ESI Accident Report Form.pdf`

**Extracted Structure:**
```
FORM TITLE: [Extract from PDF]
ACT REFERENCE: Employees State Insurance Act, 1948
RULE REFERENCE: [Extract exact rule reference]

[Structure to be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

### EPF_INSPECTION - Inspection Register

**Reference PDF:** `Forms/EPF inspection register.pdf`

**Extracted Structure:**
```
FORM TITLE: [Extract from PDF]
ACT REFERENCE: Employees Provident Funds and Miscellaneous Provisions Act, 1952
RULE REFERENCE: [Extract exact rule reference]

[Structure to be populated from reference PDF]
```

**Status:** ⏳ Awaiting reference PDF

---

## EXTRACTION CHECKLIST

For each form, verify:
- [ ] Form title matches PDF exactly (including punctuation)
- [ ] Act reference matches PDF exactly
- [ ] Rule reference matches PDF exactly
- [ ] Column count matches PDF
- [ ] Column order matches PDF (left to right)
- [ ] Column headings match PDF exactly (no paraphrasing)
- [ ] Totals row format matches PDF
- [ ] Declaration text matches PDF word-for-word
- [ ] Signature block layout matches PDF
- [ ] Page margins documented
- [ ] Font sizes documented
- [ ] Spacing documented

---

## USAGE INSTRUCTIONS

1. Place reference PDFs in `Forms`
2. For each PDF, extract structure using above template
3. Populate this document with exact structural details
4. Use populated structure to build Blade templates
5. Verify generated PDF against reference PDF visually

---

**Last Updated:** [Date]  
**Forms Mapped:** 0/36  
**Forms Pending:** 36/36
